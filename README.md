...
Connect a Vercel Blob store to production so `BLOB_STORE_ID` is available for the current OIDC connection (or
`BLOB_READ_WRITE_TOKEN` for a legacy connection). Existing public stores require no additional configuration; set
`BLOB_ACCESS=private` when connecting a private store. When the app is not running on Vercel and neither Blob
credential is configured, shared snapshots use the gitignored `.semantic-lists/` and `.personal-lists/` directories
for local development. Shared pages are unlisted and excluded from search-engine indexing. Opening a saved or shared
list never invokes the semantic-search provider; rerunning is a separate, explicit action.
