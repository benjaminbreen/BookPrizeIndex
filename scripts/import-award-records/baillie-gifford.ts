...
const pageTitle = "Baillie Gifford Prize";

async function main() {
  const registry = await readPrizeRegistry();
  const prize = registry.find((entry) => entry.id === "baillie-gifford-prize");
  const category = prize?.categories.find((entry) => entry.id === "baillie-gifford-nonfiction");
  if (!prize || !category) throw new Error("Missing baillie-gifford-prize registry entry in sources/prizes.json");

  console.log(`Fetching Baillie Gifford tables from ${pageTitle}...`);
  const wikitext = await fetchMediaWikiWikitext(pageTitle);
  const records = parseBaillieGifford(prize, category, wikitext);

  records.sort((a, b) => b.year - a.year || statusSort(a.status) - statusSort(b.status) || a.title.localeCompare(b.title));

  await writeRawAwardRecords("baillie-gifford.json", records, {
    importer: "scripts/import-award-records/baillie-gifford.ts",
    source: "MediaWiki wikitables for Baillie Gifford Prize / Samuel Johnson Prize winners and shortlists",
    notes: "The official Baillie Gifford Prize site is retained in sources/prizes.json, but this first importer uses Wikipedia as a deterministic secondary source.",
    categories: [{
  ...
  console.log(`Imported ${records.length} Baillie Gifford records.`);
  console.log(`${category.name}: ${records.length} records (${yearRange(records)})`);
}

export function parseBaillieGifford(prize: PrizeRegistryEntry, category: PrizeCategoryRegistryEntry, wikitext: string): RawAwardRecord[] {
  const tables = extractWikitables(wikitext).filter((table) => /(?:Samuel Johnson|Baillie Gifford) Prize winners? and shortlists?/i.test(table));
  if (!tables.length) throw new Error("Could not find Baillie Gifford winner/shortlist tables");
  ...
  for (const table of tables) {
    ...
    for (const row of parseWikiTableRows(table)) {
      ...
      const status = getRowStatus(row, mapped.resultRaw ?? mapped.result);
      if (!status) continue;
...
function getRowStatus(row: ParsedRow, result?: string): "winner" | "finalist" | undefined {
  if (result && /\{\{\s*(?:won|winner)\s*\}\}|^winner$/i.test(result.trim())) return "winner";
  if (result && /\{\{\s*(?:sho|shortlisted?|finalist)\s*\}\}|^(?:shortlisted?|finalist)$/i.test(result.trim())) return "finalist";
  if (/background\s*:\s*#cddeff/i.test(row.style)) return "winner";
  return undefined;
}
...
function cleanTitle(input: string) {
  return cleanText(input.replace(/\s*\([^)]*\babout\b[^)]*\)\s*/gi, " ").replace(/^"(.+)"$/, "$1"));
}
