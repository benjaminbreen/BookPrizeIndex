...
const pageTitle = "PEN/Jacqueline Bograd Weld Award";

async function main() {
  const registry = await readPrizeRegistry();
  const prize = registry.find((entry) => entry.id === "pen-weld-biography-award");
  const category = prize?.categories.find((entry) => entry.id === "pen-weld-biography");
  if (!prize || !category) throw new Error("Missing pen-weld-biography-award entry in sources/prizes.json");

  console.log(`Fetching PEN/Weld Biography table from "${pageTitle}"...`);
  ...
  await writeRawAwardRecords("pen-weld-biography.json", records, {
    importer: "scripts/import-award-records/pen-weld-biography.ts",
  ...
  console.log(`Imported ${records.length} PEN/Weld Biography records.`);
