...
const pageTitle = "Governor General's Award for English-language non-fiction";

async function main() {
  const registry = await readPrizeRegistry();
  const prize = registry.find((entry) => entry.id === "governor-generals-literary-awards");
  const category = prize?.categories.find((entry) => entry.id === "governor-generals-english-nonfiction");
  if (!prize || !category) throw new Error("Missing governor-generals-literary-awards registry entry in sources/prizes.json");

  console.log(`Fetching Governor General's Award tables from ${pageTitle}...`);
  const wikitext = await fetchMediaWikiWikitext(pageTitle);
  const records = parseGovernorGeneral(prize, category, wikitext);

  records.sort((a, b) => b.year - a.year || statusSort(a.status) - statusSort(b.status) || a.title.localeCompare(b.title));

  await writeRawAwardRecords("governor-general.json", records, {
    importer: "scripts/import-award-records/governor-general.ts",
    source: "MediaWiki decade wikitables for Governor General's Award for English-language non-fiction winners and finalists",
    notes: "The official GGBooks archive is retained in sources/prizes.json, but this first importer uses Wikipedia as a deterministic secondary source. Winners are highlighted rows; unhighlighted rows are finalists (listed from 1980 onward). Multiple winners in the 1940s reflect the era's split academic/creative non-fiction categories and are normalized as co-winners.",
    categories: [{
  ...
  console.log(`Imported ${records.length} Governor General's Award records.`);
  console.log(`${category.name}: ${records.length} records (${yearRange(records)})`);
}

export function parseGovernorGeneral(prize: PrizeRegistryEntry, category: PrizeCategoryRegistryEntry, wikitext: string): RawAwardRecord[] {
  const tables = extractWikitables(wikitext).filter((table) =>
    /Governor General's Award for English-language non-fiction winners/i.test(table));
  if (!tables.length) throw new Error("Could not find Governor General's Award winner/finalist tables");
...
function getRowStatus(row: ParsedRow): "winner" | "finalist" {
  if (/background\s*:\s*#faeb86/i.test(row.style)) return "winner";
  if (row.rawCells.some((cell) => /background\s*:\s*#faeb86/i.test(cell))) return "winner";
  return "finalist";
}

function cleanAuthorCell(input: string) {
  return stripTableBoundary(cleanText(wikiToPlainText(input)));
}

function cleanTitle(input: string) {
  return stripTableBoundary(cleanText(input.replace(/^"(.+)"$/, "$1")));
}

function stripTableBoundary(input: string) {
  return input.replace(/^\s*\|+\s*|\s*\|+\s*$/g, "").trim();
}
