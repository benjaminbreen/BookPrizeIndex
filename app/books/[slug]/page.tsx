...
import { BookDetailKeyboardNav } from "@/components/book-detail-keyboard-nav";
...
export default async function BookPage({ params }: PageProps) {
  ...
  const shelfPrevious = shelfNeighborhood?.before.at(-1);
  const shelfNext = shelfNeighborhood?.after[0];
  ...
  return (
    <main className="book-detail-page">
      ...
      <section className="book-detail-footer border-t hairline">
        <div className="mx-auto flex max-w-7xl flex-col items-center gap-4 px-4 py-8 sm:px-6 lg:px-8">
          <BookDetailKeyboardNav
            shelfNextHref={shelfNext ? `/books/${shelfNext.slug}` : undefined}
            shelfPreviousHref={shelfPrevious ? `/books/${shelfPrevious.slug}` : undefined}
            slug={book.slug}
          />
...
function ExperimentalSemanticProfilePanel({ profile }: { profile: ExperimentalSemanticProfile }) {
  ...
  const figures = mergeExperimentalEntities(profile.centralFigures, profile.lowerConfidenceCandidates?.centralFigures);
  const places = mergeExperimentalEntities(profile.centralPlaces, profile.lowerConfidenceCandidates?.centralPlaces);
  const argument = profile.argument.present
    ? { statement: profile.argument.statement, confidence: profile.argument.confidence }
    : profile.lowerConfidenceCandidates?.argument ?? null;
  return (
        ...
        <details className="experimental-profile" open>
          <summary className="experimental-profile-summary focus-ring">
            <span className="min-w-0">
              ...
              <span className="experimental-profile-disclosure">Generated by GPT-5.6 Luna · may contain inaccuracies</span>
            </span>
          ...
          <div className="experimental-profile-body">
            <p className="experimental-profile-note">
              An unverified reading of the catalog description—every field is shown with the model&apos;s own confidence, and lower numbers are more likely to be wrong.
            ...
            <ol className="experimental-profile-grid">
              <li className="experimental-profile-field">
                ...
                <ExperimentalEntityList entities={figures} />
              </li>

              <li className="experimental-profile-field">
                <p className="experimental-profile-field-label"><span>02</span> Central places</p>
                <ExperimentalEntityList entities={places} />
              </li>

              <li className="experimental-profile-field">
                <p className="experimental-profile-field-label"><span>03</span> Suggested argument</p>
                {argument ? (
                  <div className={`experimental-profile-argument ${argument.confidence < ARGUMENT_CONFIDENCE_FLOOR ? "is-low" : ""}`}>
                    <p>{argument.statement}</p>
                    <p className="plain-number experimental-profile-confidence">{confidencePercent(argument.confidence)} confidence</p>
                  </div>
                ) : (
                  <p className="experimental-profile-empty">Not extracted.</p>
                )}
              </li>
              ...
              <li className="experimental-profile-field">
                ...
                <div className="experimental-profile-orientation">
                  ...
                  <p className="experimental-profile-orientation-label">{academicOrientationLabel(profile.academicOrientation.score)}</p>
                  <p className="experimental-profile-empty">Intended readership and scholarly apparatus—not quality or importance.</p>
                </div>
              </li>
            </ol>

            <p className="experimental-profile-method">
              Percentages are the model&apos;s own estimates · profile confidence <span className="plain-number">{confidencePercent(profile.profileConfidence)}</span>
            </p>
          </div>
        </details>
      </div>
...
const ENTITY_CONFIDENCE_FLOOR = 0.7;
const ARGUMENT_CONFIDENCE_FLOOR = 0.6;

function mergeExperimentalEntities(accepted: ExperimentalSemanticEntity[], lower?: ExperimentalSemanticEntity[]) {
  const byName = new Map<string, ExperimentalSemanticEntity>();
  for (const entity of [...accepted, ...(lower ?? [])]) {
    const existing = byName.get(entity.name);
    if (!existing || entity.confidence > existing.confidence) byName.set(entity.name, entity);
  }
...
function ExperimentalEntityList({ entities }: { entities: ExperimentalSemanticEntity[] }) {
  if (!entities.length) return <p className="experimental-profile-empty">Not extracted.</p>;
  return (
    <ul className="experimental-profile-entity-list">
      {entities.map((entity) => (
        <li className={entity.confidence < ENTITY_CONFIDENCE_FLOOR ? "is-low" : ""} key={entity.name}>
          <span>{entity.name}</span>
          <span className="plain-number experimental-profile-chip">{confidencePercent(entity.confidence)}</span>
        </li>
      ))}
    </ul>
  );
